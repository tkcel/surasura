...
export function FAQ() {
  return (
    <>
      ...
      <DocsH3>Q. 録音時間に制限はありますか？</DocsH3>
      <DocsP>
        <Strong>A.</Strong> 明示的な時間制限はありません。
        録音中の音声は30秒ごとにWhisper APIへストリーミング送信されるため、音声認識自体は長時間でも動作します。
      </DocsP>
      <DocsP>
        ただし、録音データは処理完了までメモリに蓄積される仕組みのため、長時間になるほどメモリ消費が増加します。
        実用的には<Strong>15分以内</Strong>が安全圏です。
        議事録のような長時間の録音には適していませんので、短い発話を繰り返す使い方をおすすめします。
      </DocsP>

      <DocsH3>Q. テキスト入力欄以外で音声入力するとどうなりますか？</DocsH3>
      <DocsP>
        <Strong>A.</Strong> テキスト入力欄にカーソルがない状態で音声入力を行うと、
        自動ペーストの代わりにウィジェット上部に結果パネルが表示されます。
        パネルのコピーボタンから結果をクリップボードにコピーできます。
      </DocsP>
      <DocsP>
        設定画面の「自動ペースト」をオフにすると、常に結果パネルが表示されるようになります。
      ...
      <DocsP>
        <Strong>A.</Strong> アプリのサイドバーにある「このアプリについて」から最新のアップデートを確認し、ダウンロードしてください。
        破壊的な変更があった場合は動作に影響が出る可能性がありますので、その際はアプリの再インストールをお願いします。
      </DocsP>
